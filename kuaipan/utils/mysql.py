...
import _mysql
from MySQLdb.constants.CLIENT import MULTI_RESULTS, MULTI_STATEMENTS
...
class DBConnection(object):
    ...
    def __init__(self, **kw):
        ...
        self._db_params['client_flag'] = MULTI_RESULTS | MULTI_STATEMENTS
    ...
    def get_connection(self):
        if self.is_closed:
            msg = "Connection(%s) is closed, try to reconnect" % (self,)
            _logger.error(msg)
            self.reconnect()
...
def is_retryable_exception(ex):
    if isinstance(ex, OperationalError):
        if ex.args and isinstance(ex.args, (list, tuple)):
            if ex.args[0] in (
                    # Deadlock found when trying to get lock; try restarting transaction,
                    # See [http://dev.mysql.com/doc/refman/5.5/en/innodb-deadlocks.html] for more.
                    1213,
            ):
                return True
    return False
...
class QueryWrapper(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        # if lost connection, do nothing.
        if is_lost_connection_exception(exc_val):
            return None
    ...
    def session(self):
        ...
        Examples:
            ...
            with mydb.session() as conn:
                ...
                conn.execute("UPDATE xxx SET a=1;")
